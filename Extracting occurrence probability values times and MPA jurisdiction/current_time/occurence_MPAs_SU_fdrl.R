...
st_crs(UCsUS_fdrl) 
#UCsUS_fdrl <- UCsUS %>% filter(ESFERA5 == "federal")

plot(UCsUS_fdrl$geometry)
#------------

# Cutting the raster from "coral_occurrence_2020.asc" to MPAs areas
coral_occ2020_UCsUS_crop <- crop(coral_occ2020, extent(UCsUS_fdrl))
plot(coral_occ2020_UCsUS_crop)

# Converting MPAs' shapefile to raster
coral_occ2020_UCsUS_fdrl_ras <- rasterize(UCsUS_fdrl, coral_occ2020, getCover=TRUE)
coral_occ2020_UCsUS_fdrl_ras[coral_occ2020_UCsUS_fdrl_ras==0] <- NA

#Extracting and Saving occurrence probability values within MPAs
coral_occ2020_UCsUS_fdrl_mask <- mask(coral_occ2020, coral_occ2020_UCsUS_fdrl_ras, filename = "dados_sig/ucs_novas/AMPs_us/ucs_us_fdrl/coral_occ2020_UCsUS_fdrl_mask.asc")

plot(coral_occ2020_UCsUS_fdrl_mask)
#----------

# Looking at data
coral_occ2020_UCsUS_fdrl_mask <- raster ("dados_sig/ucs_novas/AMPs_us/ucs_us_fdrl/coral_occ2020_UCsUS_fdrl_mask.asc")
coral_occ2020_UCsUS_fdrl_values <- getValues(
  coral_occ2020_UCsUS_fdrl_mask )
hist(coral_occ2020_UCsUS_fdrl_values)
...
#length(na.omit(coral_occ2020_UCsUS_values)) 

#creating a data frame with the occurrence probabilities values
coral_occ2020_UCsUS_fdrl_values_df <- as.data.frame(coral_occ2020_UCsUS_fdrl_values)

#plot
windowsFonts(Times=windowsFont("Times New Roman"))

gph_UCSUS_fdrl <- ggplot(coral_occ2020_UCsUS_fdrl_values_df, 
       aes(coral_occ2020_UCsUS_fdrl_values)) + 
