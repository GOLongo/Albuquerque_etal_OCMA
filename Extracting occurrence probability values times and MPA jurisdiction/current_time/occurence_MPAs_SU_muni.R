...
st_crs(UCsUS_muni)
#UCsUS_muni <- UCsUS %>% filter(ESFERA5 == "municipal")

plot(UCsUS_muni$geometry)
#----------------

# Cutting the raster from "coral_occurrence_2020.asc" to MPAs areas
coral_occ2020_UCsUS_crop <- crop(coral_occ2020, extent(UCsUS_muni))
plot(coral_occ2020_UCsUS_crop)

# Converting MPAs' shapefile to raster
coral_occ2020_UCsUS_muni_ras <- rasterize(UCsUS_muni, coral_occ2020, getCover=TRUE)
coral_occ2020_UCsUS_muni_ras[coral_occ2020_UCsUS_muni_ras==0] <- NA

#Extracting and Saving occurrence probability values within MPAs
coral_occ2020_UCsUS_muni_mask <- mask(coral_occ2020, coral_occ2020_UCsUS_muni_ras, filename = "dados_sig/ucs_novas/AMPs_us/ucs_us_muni/coral_occ2020_UCsUS_muni_mask.asc")

plot(coral_occ2020_UCsUS_muni_mask)
#----------
...
gph_UCSUS_muni <- ggplot(coral_occ2020_UCsUS_muni_values_df, 
       aes(coral_occ2020_UCsUS_muni_values)) + 
...
gph_UCSUS_fdrl /gph_UCSUS_estd / gph_UCSUS_muni
