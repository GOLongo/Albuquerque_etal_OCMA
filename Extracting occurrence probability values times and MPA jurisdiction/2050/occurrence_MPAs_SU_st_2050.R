...
# sustainable use  MPAs under state jurisdiction #
# in 2040-2050                                   #
##################################################
...
UCsUS_estd <-st_read(dsn = "dados_sig/ucs_novas/AMPs_us/ucs_us_estd/ucs_us_estd.shp", layer = 'ucs_us_estd', crs = 4326)
st_crs(UCsUS_estd) 
#UCsUS_estd <- UCsUS %>% filter(ESFERA5 == "estadual")

plot(UCsUS_estd$geometry)

#----------------

# Cutting the raster from "coral_occurrence_2020.asc" to MPAs areas
coral_occ2050_UCsUS_crop <- crop(coral_occ2050, extent(UCsUS_estd))
plot(coral_occ2050_UCsUS_crop)

# Converting MPAs' shapefile to raster
coral_occ2050_UCsUS_estd_ras <- rasterize(UCsUS_estd, coral_occ2050, getCover=TRUE)
coral_occ2050_UCsUS_estd_ras[coral_occ2050_UCsUS_estd_ras==0] <- NA

#Extracting and Saving occurrence probability values within MPAs
coral_occ2050_UCsUS_estd_mask <- mask(coral_occ2050, coral_occ2050_UCsUS_estd_ras, filename = "dados_sig/2050/UCsUS/UCsUS_estd/coral_occ2050_UCsUS_estd_mask.asc", overwrite = T)

plot(coral_occ2050_UCsUS_estd_mask)
#----------
...
#length(na.omit(coral_occ2050_UCsUS_values)) # Aqui, s? para visualizar o numero de USxels em UCsUS
...
gph_UCSUS_estd_2050 <- ggplot(coral_occ2050_UCsUS_estd_values_df, 
                         aes(coral_occ2050_UCsUS_estd_values)) + 
