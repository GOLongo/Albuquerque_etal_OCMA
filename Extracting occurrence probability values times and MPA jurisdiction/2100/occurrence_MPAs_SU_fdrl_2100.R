...
coral_occ2100_UCsUS_crop <- crop(coral_occ2100, extent(UCsUS_fdrl))
plot(coral_occ2100_UCsUS_crop)

# Converting MPAs' shapefile to raster
coral_occ2100_UCsUS_fdrl_ras <- rasterize(UCsUS_fdrl, coral_occ2100, getCover=TRUE)
coral_occ2100_UCsUS_fdrl_ras[coral_occ2100_UCsUS_fdrl_ras==0] <- NA

#Extracting and Saving occurrence probability values within MPAs
coral_occ2100_UCsUS_fdrl_mask <- mask(coral_occ2100, coral_occ2100_UCsUS_fdrl_ras, filename = "dados_sig/2100/UCsUS/UCsUS_fdrl/coral_occ2100_UCsUS_fdrl_mask.asc", overwrite = T)

plot(coral_occ2100_UCsUS_fdrl_mask)
#----------

# Looking at data
coral_occ2100_UCsUS_fdrl_mask <- raster ("dados_sig/2100/UCsUS/UCsUS_fdrl/coral_occ2100_UCsUS_fdrl_mask.asc")
coral_occ2100_UCsUS_fdrl_values <- getValues(
  coral_occ2100_UCsUS_fdrl_mask )
hist(coral_occ2100_UCsUS_fdrl_values)
...
#length(na.omit(coral_occ2100_UCsUS_values)) # Aqui, s? para visualizar o numero de USxels em UCsUS


#creating a data frame with the occurrence probabilities values
coral_occ2100_UCsUS_fdrl_values_df <- as.data.frame(coral_occ2100_UCsUS_fdrl_values)
hist(coral_occ2100_UCsUS_fdrl_values)

#plot
windowsFonts(Times=windowsFont("Times New Roman"))

gph_UCSUS_fdrl_2100 <- ggplot(coral_occ2100_UCsUS_fdrl_values_df, 
                              aes(coral_occ2100_UCsUS_fdrl_values)) + 
